...
class IzNetwork:
  ...
  def NeuronUpdate(self, i, t):
    ...
    for j in xrange(self.Nlayers):
	  ...
	  # firings[k-1, 1] is the nueron number, so, delay is associating
	  # a delay to a neuron number
	  # firings[k-1, 0] is the time right now, minus the time
	  # that the neuron fire. If they are equal, it means we should be
	  # expecting it, as that is the delay we generate for it, so it
	  # produces an array of bools, to say which neuarons should fire
	  # this time round. 
          idx = delay[:, firings[k-1, 1]] == (t-firings[k-1, 0])
	  # So if hte boolean is true at that index it will update the current array.
	  # S is the connectivity matrix
    ...
    for k in xrange(int(1/dt)):
      ...
      # As specified in teh lecture notes update the membrane potential, and the
      # recovery variable.
      ...
      # 30 is the threshold for the neuron firing.
      # Goes through each layer checking which fired on each layer.
      fired = np.where(self.layer[i].v >= 30)[0]

      if len(fired) > 0:
        for f in fired:
          # Add spikes into spike train
	  # Checking for firings at each layer. 
